# dotfiles/bash/custom/exiftool.sh
...
then
  ...
  function exifSetDate() {
    ...
    if [[ ${#time_check} -ne 1 ]]
    then
      printf "%sError:%s Time format is HH:MM\n" "$RED_BOLD" "$RESET" && return 1
    fi
    ...
    case $zone in
      ...
      PST)
        ...
        ;;
      *)
        printf "%sError:%s Unknown Time Zone%s\n" "$RED_BOLD" "$RESET" "$zone" && return 1
    esac
